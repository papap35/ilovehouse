...
- **實價登錄查詢**（`/search`）：依縣市、行政區、交易類型（買賣／租賃）、價格、坪數、建物型態等條件篩選成交行情，並顯示筆數、平均總價、平均單價等統計。可切換顯示地圖：地圖以「行政區中心點」聚合標記均價與樣本數，也可在地圖上點一下設定圓心、用滑桿調整半徑做「畫圈範圍搜尋」，範圍內（可跨縣市）的行政區會一起納入查詢結果。
...
- `lib/districtGeo.ts` / `lib/geo.ts` / `components/PropertyMap.tsx`：地圖顯示與範圍搜尋。用 [Leaflet](https://leafletjs.com/) + OpenStreetMap 圖磚（免費、不需 API 金鑰）。因為實價登錄資料沒有精確經緯度，`districtGeo.ts` 內建每個行政區的概略中心點座標（依一般地理知識估算，非正式地理編碼），地圖上的標記與「畫圈搜尋」都是以行政區為單位聚合，不是逐筆物件的精確定位。
...
lib/
  ...
  listingLinks.ts             導向房仲平台搜尋結果的連結產生器
  districtGeo.ts               行政區概略中心點座標表
  geo.ts                        haversine 距離計算／範圍內行政區搜尋
tests/                     vitest 單元測試
```
